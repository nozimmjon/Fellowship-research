...
**Interpretation rule:** rank-based results carry more weight than 2022 category rates due to denominator size.
...
- 2022 category persistence is high (`r fmt_pct(metric_est("persistence_probability", 2022))`) but based on only `r metric_n("upward_mobility_rate", 2022)` valid category observations.
- **Main read:** descriptive persistence rises across waves; categorical 2022 levels are directional only.
:::
::: {.column width="42%"}
...
- Parent-rank interactions for 2016 and 2022 are imprecise in pooled trend models.
...
**Main read:** strongest evidence is on persistence levels and parental-background correlates, not precise trend change.
...
- Descriptive persistence rises across waves, but pooled models do not estimate the change precisely.
- 2022-23 category-based mobility rates use a much smaller valid denominator than rank-based estimates.
...
- 2022 valid N: `r metric_n("upward_mobility_rate", 2022)` (category rates) vs `r metric_n("rank_rank_slope", 2022)` (rank-rank slope).
:::
::: {.column width="42%"}
