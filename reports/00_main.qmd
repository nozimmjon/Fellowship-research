...
The interaction pattern differs from the earlier draft. The urban interaction is close to zero and not precisely estimated, while the female interaction is negative at **`r fmt_num(coef_est("eq5_persistence_heterogeneity", "parent_ed_score:female"))`** with p-value **`r fmt_p(coef_p("eq5_persistence_heterogeneity", "parent_ed_score:female"))`**. The interaction between parent education and the 2022 wave is positive at **`r fmt_num(coef_est("eq5_persistence_heterogeneity", "parent_ed_score:wave2022"))`** and precisely estimated. A cautious reading is that parental education translates into same-category persistence across the pooled sample, with a stronger gradient in 2022-23 and weaker transmission among women in this specification. Table @tbl-pooled-correlates collects the selected coefficients discussed in Section 6.
...
The technical appendix shows that the stoppage coefficient is fragile across weighting and threshold choices. Under the strict `<=9` split, the low-parent-education group falls to **`r fmt_int(robust_scenario_value("weighted_leq9", "n_low_group"))`** observations (vs **`r fmt_int(robust_scenario_value("weighted_leq9", "n_high_group"))`** in the comparison group), so this scenario is treated as degenerate support (`r robust_scenario_value("weighted_leq9", "scenario_status")`) rather than an interpretable robustness estimate. Module C therefore contributes bounded descriptive evidence on household vulnerability and learning support, not a stable adjusted parental-education gradient and not causal evidence on pandemic mechanisms.
